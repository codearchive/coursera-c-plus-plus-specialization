...
{
    int counter = 1;
    map<Region, int> unique_regions;
    if (regions.empty()) { return 0; }
    for (size_t i = 0; i < regions.size(); ++i)
    {
        unique_regions[regions[i]]++;
    }
    for (const auto& [region, region_counter] : unique_regions)
    {
        if (region_counter > counter) { counter = region_counter; }
    }
    return counter;
}
